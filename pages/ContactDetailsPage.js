...
export const deleteAllContacts = async (page) => {
	let elementCount = await page
		.locator('//tr[@class="contactTableBodyRow"]')
		.count();
	while (elementCount !== 0) {
		...
		await expect(
			page.locator('//tr[@class="contactTableBodyRow"]'),
		).toHaveCount(elementCount - 1);
		elementCount = await page
			.locator('//tr[@class="contactTableBodyRow"]')
			.count();
		elementCount > 0
			? console.log(`${elementCount} contact(s) remaining`)
